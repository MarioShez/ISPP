...
public class RouteCreationTest extends AbstractTest {
	...
	public void WithoutIntermediateCPTest() {
		...
		Route r = this.routeService.create();
		RouteForm rf = this.routeService.construct(r);
		...
		for (Vehicle v : this.driver.getVehicles()) {
			vehicle = v;
		}
		...
		r = this.routeService.reconstruct(rf, this.driver, null);
		this.routeService.calculaDuracion(r);
		r = this.routeService.save2(r);

		Assert.notNull(r);

		r = this.routeService.findOne(r.getId());

		Assert.notNull(r);
		Assert.isTrue(r.getDriver().getId() == this.driver.getId());
		Assert.isTrue(r.getAvailableSeats() == 3);
		Assert.isTrue(r.getDistance() > 0d);
		Assert.isTrue(r.getPricePerPassenger() >= 1.25d);
		...
		List<ControlPoint> cps = this.cpService.findByRoute(r);
	...
	public void WithIntermediateCPTest() {
		...
		this.cpService.addControlPoint(rf);
	...
	public void WithIntermediateCPAndDeletionsTest() {
		...
		this.cpService.removeControlPoint(rf, 0);
	...
	@Before
	public void setUp() {
		Driver driver = this.driverService.create();
		...
		driver.setUserAccount(this.hashSavePassword(driver.getUserAccount()));
		driver = this.driverService.save(driver);
		...
		this.driver = (Driver) this.actorService.findByPrincipal();
